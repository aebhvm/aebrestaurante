...
export async function GET(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!Number.isInteger(id) || id <= 0) return NextResponse.json({ error: "Ficha inválida." }, { status: 400 });
  ...
  if (!photoUrl) return NextResponse.json({ error: "Foto não encontrada." }, { status: 404 });

  if (photoUrl.startsWith("data:")) {
    const match = /^data:([^;,]+);base64,(.*)$/s.exec(photoUrl);
    if (!match) return NextResponse.json({ error: "Foto inválida." }, { status: 422 });

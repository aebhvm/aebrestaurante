...
function BreakForm({ employees, date, breakItem }: { employees: Employee[]; date: string; breakItem?: BreakItem }) {
  ...
  return (
    <form action={breakItem ? updateBreakAction : createBreakAction} className="space-y-3">
      ...
      <div className="space-y-2"><Label>Início</Label><Input name="startsAt" type="time" defaultValue={breakItem?.startsAt} required /></div>
      <p className="text-xs text-muted-foreground">O descanso termina automaticamente uma hora após o início.</p>
